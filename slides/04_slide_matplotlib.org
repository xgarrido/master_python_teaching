...
:END:
#+TOC: headlines
** =matplotlib= \faIcon{question-circle}
...
#+BEGIN_SRC python
  ...
  In [8]: plt.plot(x, x + 1, linestyle="dashed")

  In [9]: plt.plot(x, x + 2, linestyle="dashdot")

  In[10]: plt.plot(x, x + 3, linestyle="dotted")
#+END_SRC
...
- Code héxadécimal (=RRGGBB=)
  #+ATTR_LATEX: :options fontsize=\scriptsize
  #+BEGIN_SRC python
    In[11]: plt.plot(x, np.sin(x - 3), color="#FFDD44")
  #+END_SRC
- RGB /tuple/ =[0; 1]=
  #+ATTR_LATEX: :options fontsize=\scriptsize
  #+BEGIN_SRC python
    In[12]: plt.plot(x, np.sin(x - 4), color=(1.0,0.2,0.3))
  #+END_SRC
...
#+ATTR_LATEX: :options fontsize=\scriptsize
#+BEGIN_SRC python
  In [3]: plt.hist(data, bins=30, density=True)
#+END_SRC
