...
- =numpy= fournit essentiellement des objets de type *vecteurs, matrices en généralisant la notion de
...
- Documentation de référence du module https://numpy.org/doc
...
- Du fait de cette hétérogénéité, les listes ne supportent pas les opérations mathématiques

- Les tableaux ou /array/ de =numpy= sont *statiquement typées et homogènes[fn:ea2b86f523276c9]*

  - Le type des éléments est déterminé lorsque le tableau est créé *\to plus de typage dynamique*
  - De même la taille du tableau est fixée à la création *\to stockage en mémoire optimisé*

- En raison du typage statique, les opérations mathématiques peuvent être mises en œuvre /via/ un
  langage compilé (C et Fortran)

#+BEGIN_COMMENT
So far the numpy.ndarray looks awefully much like a Python list (or nested list). Why not simply use
...
- Démonstration[fn:1bc7c507a7609f02]
  #+BEGIN_SRC python
...
- Vecteurs, matrices avec valeurs par défaut
  #+BEGIN_SRC python
    ...
    In [3]: np.full(shape=(3, 3), fill_value=666.666, dtype=int)
    Out[3]:
    array([[666, 666, 666],
           [666, 666, 666],
           [666, 666, 666]])
...
- La fonction =np.diag_indices= retourne les couples de valeurs d'indices correspondant à la diagonale
  d'une matrice carrée tandis que la fonction =np.diag= extrait la diagonale
...
- Toutefois, la formulation =np.sum()= propre à =numpy= présente l'avantage d'être
  ...
  #+BEGIN_SRC python
    In [4]: big_array = np.random.rand(1_000_000)
...
- Somme des éléments d'un tableau : méthode =sum=
  #+BEGIN_SRC python
    In [1]: M = np.random.randint(10, size=(3, 4))
    In [2]: M
    Out[2]:
    array([[7, 0, 8, 4],
           [4, 7, 0, 5],
...
- Il est finalement possible de dénombrer le nombre de valeurs d'un tableau
  ...
  #+BEGIN_REMARK
  Il n'est pas possible d'utiliser les opérateurs =or=, =and= ou =not=. Ces opérateurs sont
  respectivement remplacés par =|=, =&= et =~= (/bit logic/)
  #+END_REMARK
...
- *le module [[http://pandas.pydata.org/][=pandas=]] est toutefois bien mieux adapté à la lecture/écriture de fichier contenant des
  données numériques*
...
[fn:1bc7c507a7609f02] =%timeit= est une commande /magique/ de =ipython=
