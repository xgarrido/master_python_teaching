...
- =numpy= fournit en particulier des objets de type *vecteurs, matrices et en généralisant la notion
  de tableaux à $n$ dimensions \to =ndarray=*
...
- Démonstration
  ...
  #+BEAMER: \pause
  #+BEGIN_SRC python
    In [2]: %timeit a = np.arange(1000); a**2
...
* Manipulation de données avec =pandas=                               :title:
:PROPERTIES:
:BEAMER_ENV: fullframe
:END:

#+BEGIN_EXPORT beamer
\title{Manipulation de données avec \texttt{pandas}}
\titlepage
#+END_EXPORT
